document.addEventListener('DOMContentLoaded', async () => {
    await App.loadPartial('#admin-panel-placeholder', 'partials/admin-panel.html');
    await App.loadPartial('#routines-placeholder', 'partials/manage-routines.html');
    document.getElementById('retrieve-form').addEventListener('submit', submit_musics)
});


function submit_musics(e){
    e.preventDefault();
    ...
    fetch(`/api/get_videos/${encodeURIComponent(search)}`)
    .then(response => {
            if (!response.ok) {
